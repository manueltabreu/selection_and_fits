...
#     pdfstring = "fitfunction%s_Norm[tagged_mass]_Range[datarange]_NormRange[datarange]"%(ibin)  ## sara 04.11 to comment
    ...
    for ilog in [False]:
        ...
        c1.SaveAs('fit_results_mass_checkOnMC/newbdt_puw/QUALITYsave_fit_mc_%s_%s_%s_newSigmaFRT%s%s_asInANv8.pdf'%(ibin, args.year, tag, '_logScale'*ilog, '_Jpsi'*(args.dimusel=='keepJpsi')))
...
#    fitFunction.paramOn(frame, RooFit.Parameters(parList2), RooFit.Layout(0.72,0.86,0.89)) 
    ...
    for ilog in [False]:
        upperPad.SetLogy(ilog)
        c1.SaveAs('fit_results_mass_checkOnMC/newbdt_puw/QUALITYsave_fit_data_%s_%s_LMNR_newSigmaFRT_%s_asInANv8.pdf'%(ibin, args.year, '_logScale'*ilog))
